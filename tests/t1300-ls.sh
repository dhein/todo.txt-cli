...
test_description='basic list functionality
...
#
TEST_TODO_=`pwd`/todo.cfg
TEST_TODO1_=`pwd`/todo1.cfg
TEST_TODO2_=`pwd`/todo2.cfg
sed -e "s/^.*export TODOTXT_SORT_COMMAND=.*$/export TODOTXT_SORT_COMMAND='env LC_COLLATE=C sort -f -k2 | tail -r'/" "${TEST_TODO_}" > "${TEST_TODO1_}"
